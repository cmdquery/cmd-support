class Integrations::Slack::SendPendingAlertService
  ...
  def perform
    ...
    return if conversation.additional_attributes['pending_alert_thread_ts'].present?

    response = slack_client.chat_postMessage(
      channel: hook.reference_id,
      text: alert_text
    )
    conversation.update!(additional_attributes: conversation.additional_attributes.merge('pending_alert_thread_ts' => response['ts']))
